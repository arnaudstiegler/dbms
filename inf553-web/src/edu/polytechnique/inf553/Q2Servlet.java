	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			sql = "SELECT country.name, COUNT(artist.id) "
					+ "FROM country INNER JOIN artist ON artist.area = country.id " + "GROUP BY country.id "
			...
			while (rs.next()) {
				...
				writer.append("</country>").append("<number>").append(StringEscapeUtils.escapeXml11(number))
						.append("</number>");
				writer.append("</row>");
