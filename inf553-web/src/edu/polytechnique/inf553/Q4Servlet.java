...
public final class Q4Servlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			String sql;
			sql = "SELECT release.id,release.title FROM ((artist LEFT JOIN release_has_artist r ON artist.id = r.artist) LEFT JOIN release_country ON release_country.release = r.release) LEFT JOIN release ON release.id=r.release "
					+ "WHERE  artist.name='" + name + "' AND " + "((release_country.year>" + ybegin
					+ ") OR  (release_country.year<" + mend + ")" + " OR (release_country.year=" + ybegin
					+ " AND release_country.month>" + mbegin + ")" + " OR (release_country.year=" + ybegin
					+ " AND release_country.month=" + mbegin + "AND release_country.day>=" + dbegin + ")"
					+ " OR (release_country.year=" + yend + " AND release_country.month<" + mend + ")"
					+ " OR (release_country.year=" + yend + " AND release_country.month=" + mend
					+ "AND release_country.day<=" + dend + "))"
